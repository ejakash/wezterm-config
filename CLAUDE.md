...
- **Plugin** = `local M = {}; function M.apply(config, ctx) ... end; return M`. Loaded sorted, pcall-isolated, after all base sections. `ctx` = `{ wezterm, machine, theme, render_segment, add_tab_overlay, add_status_cell }` (`render_segment` builds a theme-consistent status segment — design-agnostic name; pill shape lives in the one function). Full contract: `plugins/README.md`.
